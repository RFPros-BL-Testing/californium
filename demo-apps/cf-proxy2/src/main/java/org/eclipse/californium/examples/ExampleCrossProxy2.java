...
public class ExampleCrossProxy2 {
	...
	public ExampleCrossProxy2(Configuration config, boolean accept, boolean cache) throws IOException {
		...
		coapsPort = config.get(CoapConfig.COAP_SECURE_PORT);
		...
		//JK: Adding DTLS Connector for a coaps endpoint
		coapProxyServer.add(new CoapResource("secure") {
		...
		coapProxyServer.addEndpoint(coapBuilder.build());
